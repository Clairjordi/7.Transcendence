...
import { GameCustom } from "./ClassGameCustom";
...
export const Custom = () => {
    ...
    var gameplayref = useRef<GameCustom | null>(null);
    let status: string = '';
    ...
    useEffect(() => {
        if (!gameplayref.current && otherPlayer) {
            gameplayref.current = new GameCustom(gameref, otherPlayer, status, setExit, setEnd, setNamePlayer);
    ...
    return (
        ...
        <div className="flex items-center justify-end">
            ...
            <div className="p-2 bg-cover bg-fixed" style={{ backgroundImage: `url(${require('../../img/bg-game.jpg')})` }}>
                <div className="p-1.5 bg-gradient-to-r from-green-400 via-pink-500 to-purple-500 w-auto m-auto overscroll-contain" style={{ height: '100%', position: 'relative' }}>
                    ...
                    <div className="absolute h-[95.5%] md:h-[97.5%] xl:h-[98.5%] w-1/2 border-r-4 border-dashed border-white">
                        {endGame === 1 && <div className="absolute top-2/3" style={{ left: '85%', transform: 'translateX(-80%)' }}>
                            <button onClick={gameplayref.current?.clickRetry} className="text-white text-sm bg-slate-600 font-semibold px-4 py-2 rounded-lg hover:scale-110">
                                RETRY
                            </button>
                        </div>}
                    </div>
                    <div className="float-right w-1/2">
                        ...
                        {endGame === 1 && <div className="absolute top-[65.9%] xl:top-[66.3%]" style={{ right: '42%', transform: 'translateX(80%)' }}>
                            <button onClick={gameplayref.current?.clickHandler} className="text-white text-sm bg-slate-600 font-semibold px-4 py-2 rounded-lg hover:scale-110">
                                QUIT
                            </button>
                        </div>}
                    </div>
                    <canvas className="w-[100%] flex flex-wrap overflow-auto overscroll-contain m-auto bg-cover bg-fixed" style={{ backgroundImage: `url(${require('../../img/bg-game.jpg')})` }} width={1500} height={700} ref={gameref} />
                </div>
            </div>
        </div>
    );
